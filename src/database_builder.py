...
def build_face_database():
    ...
    if not os.path.exists(Config.csv_info_file):
        print("[ERROR] Không tìm thấy file data.csv!")
        return

    df = pd.read_csv(Config.csv_info_file)
    
    # Kiểm tra cột bắt buộc (đã bỏ House)
    required_cols = ["ID_Name", "Name"]
    if not all(col in df.columns for col in required_cols):
        print(f"[ERROR] File data.csv thiếu cột bắt buộc: {required_cols}")
        return
    ...
    df["embedding_index"] = df["ID_Name"].apply(
        lambda pid: next((i for i, label in enumerate(labels) if label["ID_Name"] == pid), None)
    )
    
    cols_to_save = ["ID_Name", "Name", "embedding_index"]
    existing_cols = [c for c in cols_to_save if c in df.columns]
    df[existing_cols].to_csv(Config.csv_info_file, index=False, encoding="utf-8-sig")
